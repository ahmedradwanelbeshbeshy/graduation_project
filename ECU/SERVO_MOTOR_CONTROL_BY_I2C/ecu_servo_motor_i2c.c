...
Std_ReturnType ServoDriver_Init(const mssp_i2c_st *i2c_obj,const servo_driver_st *servo_driver_obj,uint8*ack)
{
    ...
    else
    {
     ...
     PCA_Write_Reg(i2c_obj,servo_driver_obj->slave_address,ECU_SM_PRE_SCALE_REG,servo_driver_obj->frequancy,ack);
     PCA_Write_Reg(i2c_obj,servo_driver_obj->slave_address,ECU_SM_MODE1_REG,servo_driver_obj->mode_1_cfg,ack);
     PCA_Write_Reg(i2c_obj,servo_driver_obj->slave_address,ECU_SM_MODE2_REG,servo_driver_obj->mode_2_cfg,ack);
